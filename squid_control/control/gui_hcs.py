...
class OctopiGUI(QMainWindow):
    ...
    def __init__(self, is_simulation=False, *args, **kwargs):
        ...
        if CONFIG.ENABLE_TRACKING:
            ...
            self.imageDisplayWindow = core.ImageDisplayWindow(
                draw_crosshairs=True, show_LUT=True, autoLevels=True
            )
        ...
        self.navigationViewer = core.NavigationViewer(
            sample=str(CONFIG.WELLPLATE_FORMAT) + " well plate"
        )
        """
        if CONFIG.HOMING_ENABLED_Z:
        ...
        if CONFIG.HOMING_ENABLED_Z and HOMING_ENABLED_X and CONFIG.HOMING_ENABLED_Y:
        ...
        if CONFIG.ENABLE_SPINNING_DISK_CONFOCAL:
            self.spinningDiskConfocalWidget = widgets.SpinningDiskConfocalWidget(
                self.xlight, self.configurationManager
            )

        if CONFIG.CAMERA_TYPE == "Toupcam":
        ...
        self.navigationWidget = widgets.NavigationWidget(
            ...
            widget_configuration="384 well plate",
        )
        ...
        self.wellSelectionWidget = widgets.WellSelectionWidget(CONFIG.WELLPLATE_FORMAT)
        self.scanCoordinates.add_well_selector(self.wellSelectionWidget)

        if CONFIG.ENABLE_FLEXIBLE_MULTIPOINT:
            self.recordTabWidget.addTab(self.multiPointWidget2, "Flexible Multipoint")
        if CONFIG.ENABLE_SPINNING_DISK_CONFOCAL:
            self.recordTabWidget.addTab(
                self.spinningDiskConfocalWidget, "Spinning Disk Confocal"
            )
        ...
        if SINGLE_WINDOW:
            ...
            dock_wellSelection.setFixedHeight(
                dock_wellSelection.minimumSizeHint().height()
            )
        ...
        try:
            self.cswfcWindow = widgets.WrapperWindow(
                self.cameraSettingWidget_focus_camera
            )
        except AttributeError:
            pass
        ...
        self.wellSelectionWidget.signal_wellSelectedPos.connect(
            self.navigationController.move_to
        )
        ...
        if CONFIG.SUPPORT_LASER_AUTOFOCUS:
            ...
            # widgets
            if CONFIG.FOCUS_CAMERA_TYPE == "Toupcam":
            ...
            self.laserAutofocusControlWidget = widgets.LaserAutofocusControlWidget(
                self.laserAutofocusController
            )

            self.recordTabWidget.addTab(
                self.laserAutofocusControlWidget, "Laser Autofocus Control"
            )
            ...
            dock_laserfocus_liveController = dock.Dock(
                "Focus Camera Controller", autoOrientation=False
            )
            dock_laserfocus_liveController.showTitleBar()
            dock_laserfocus_liveController.addWidget(
            ...
            dock_laserfocus_liveController.setFixedWidth(
                self.liveControlWidget_focus_camera.minimumSizeHint().width()
            )
            ...
            if CONFIG.SHOW_LEGACY_DISPLACEMENT_MEASUREMENT_WINDOWS:
                laserfocus_dockArea.addDock(
            ...
            self.displacementMeasurementController.signal_plots.connect(
                self.waveformDisplay.plot
            )
            self.displacementMeasurementController.signal_readings.connect(
                self.displacementMeasurementWidget.display_readings
            )
            self.laserAutofocusController.image_to_display.connect(
                self.imageDisplayWindow_focus.display_image
            )
    ...
    def closeEvent(self, event):
        ...
        self.navigationController.move_x(
            0.1
        )  # temporary bug fix - move_x needs to be called before move_x_to if the stage has been moved by the joystick
        while self.microcontroller.is_busy():
            time.sleep(0.005)
        ...
        self.navigationController.move_y(
            0.1
        )  # temporary bug fix - move_y needs to be called before move_y_to if the stage has been moved by the joystick
        while self.microcontroller.is_busy():
            time.sleep(0.005)
