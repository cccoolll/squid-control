...
async def start_tile_service(server_url):  
    ...
    def get_metadata():  
        """Return metadata about the available tiles and image properties"""  
        zarr_path = os.path.join(ZARR_FOLDER, f"{CHANNEL_NAME}.zarr")  
        if not os.path.exists(zarr_path):  
            raise FileNotFoundError(f"Channel '{CHANNEL_NAME}' not found.")  
    ...
    # Register the regular service  
    svc = await server.register_service({  
        "name": "Microscopy Tile Service",  
        "id": "microscopy-tile-service",  
        "config": {  
            "visibility": "public",  
            "description": "Serves image tiles from Zarr files for microscopy data"  
        },  
    ...
    print(f"Services registered at workspace: {server.config.workspace}")  
    print(f"Regular service ID: {svc.id}")  
    print(f"WebRTC service ID: microscopy-tile-stream")  
