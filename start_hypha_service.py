...
class Microscope:
            ...
            try:
                ...
                if not chatbot_token:
                    ...
                    chatbot_server = await connect_to_server({
                        "client_id": f"squid-chatbot-{self.service_id}",
    ...
    async def connect_to_similarity_search_service(self):
        if self.is_local:
            similarity_search_server = await connect_to_server(
                {"client_id": f"similarity-search-{self.service_id}", "server_url": "http://192.168.2.1:9527", "token": os.environ.get("REEF_LOCAL_TOKEN"), "workspace": os.environ.get("REEF_LOCAL_WORKSPACE"), "ping_interval": None}
            )
            similarity_search_svc = await similarity_search_server.get_service("image-text-similarity-search")
        else:
            similarity_search_server = await connect_to_server(
                {"client_id": f"similarity-search-{self.service_id}", "server_url": "https://hypha.aicell.io", "token": os.environ.get("AGENT_LENS_WORKSPACE_TOKEN"), "workspace": "agent-lens", "ping_interval": None}
            )
            similarity_search_svc = await similarity_search_server.get_service("image-text-similarity-search")
    ...
    async def setup(self):
        ...
        chatbot_server = await connect_to_server({"client_id": f"squid-chatbot-{self.service_id}", "server_url": chatbot_server_url, "token": chatbot_token,  "ping_interval": None})
